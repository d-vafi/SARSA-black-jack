...
class GameState:
    ...
    def place_bet(self):
        if not self.can_play():
            ...
            print(f"[BET] Cannot place bet - Budget: ${self.budget}, Required: ${self.bet_amount}")
            return False
        
        self.budget -= self.bet_amount
        self.current_bet = self.bet_amount
        print(f"[BET] Bet placed: ${self.current_bet} | Remaining budget: ${self.budget}")
        return True
    ...
    def deal_initial_hand(self):
        ...
        print(f"[DEAL] Player cards: {card1}, {card3} | Value: {self.player_hand.get_value()}")
        print(f"[DEAL] Dealer shows: {card4} | Hidden card: [?]")
    ...
    def player_hit(self):
        ...
        print(f"[{hand_label.upper()}] Drew: {card} | New value: {hand_value}")
        
        if active_hand.is_bust():
            print(f"[BUST] {hand_label} busted with {hand_value}!")
            
            if self.is_split:
                # Move to next hand or end if this was the last hand
                if self.current_hand_index < len(self.split_hands) - 1:
                    self.current_hand_index += 1
                    print(f"[SPLIT] Moving to Hand {self.current_hand_index + 1}")
                    return False
                else:
                ...
                self.losses += 1
                print(f"[RESULT] Dealer Wins | Budget: ${self.budget} | W/L/D: {self.wins}/{self.losses}/{self.draws}")
                self.current_bet = 0
                return True
        
        # Check if hand reached 21
        if hand_value == 21:
            print(f"[{hand_label.upper()}] Reached 21!")
            if self.is_split:
                if self.current_hand_index < len(self.split_hands) - 1:
    ...
    def split_hand(self):
        ...
        print(f"[SPLIT] Splitting pair of {self.player_hand.cards[0].rank}")
        
        # Deduct additional bet
        self.budget -= self.bet_amount
        print(f"[SPLIT] Additional bet placed: ${self.bet_amount} | Remaining budget: ${self.budget}")
        ...
        print(f"[SPLIT] Hand 1: {hand1.cards[0]}, {card1} | Value: {hand1.get_value()}")
        print(f"[SPLIT] Hand 2: {hand2.cards[0]}, {card2} | Value: {hand2.get_value()}")
        print("[SPLIT] Now playing Hand 1")
    ...
    def dealer_play(self):
        ...
        print(f"\n[DEALER] Reveals hand | Value: {dealer_value}")
    ...
    def determine_winner(self):
        ...
        if self.is_split:
            ...
            for i, hand in enumerate(self.split_hands):
                ...
                print(f"\n[SHOWDOWN] Hand {i + 1}: {hand_value} vs Dealer: {dealer_value}")
                
                if hand.is_bust():
                    results.append(f"Hand {i + 1}: BUST")
                    self.losses += 1
                    print(f"[RESULT] Hand {i + 1} BUSTED | Lost ${bet}")
                elif self.dealer_hand.is_bust():
                    results.append(f"Hand {i + 1}: WIN")
                    payout = bet * 2
                    total_payout += payout
                    self.wins += 1
                    print(f"[RESULT] Hand {i + 1} Wins (Dealer Bust) | Won ${payout}")
                elif hand_value > dealer_value:
                    results.append(f"Hand {i + 1}: WIN")
                    payout = bet * 2
                    total_payout += payout
                    self.wins += 1
                    print(f"[RESULT] Hand {i + 1} Wins | Won ${payout}")
                elif hand_value < dealer_value:
                    results.append(f"Hand {i + 1}: LOSE")
                    self.losses += 1
                    print(f"[RESULT] Hand {i + 1} Loses | Lost ${bet}")
                else:
                    results.append(f"Hand {i + 1}: PUSH")
                    total_payout += bet
                    self.draws += 1
                    print(f"[RESULT] Hand {i + 1} Push | Returned ${bet}")
            ...
            print(f"\n[SHOWDOWN] Player: {player_value} vs Dealer: {dealer_value}")
            ...
            if self.player_hand.is_blackjack() and not self.dealer_hand.is_blackjack():
                ...
                self.wins += 1
                print(f"[RESULT] BLACKJACK! Player Wins ${winnings}")
            elif self.dealer_hand.is_bust():
                self.result_message = "Dealer Busts! You Win!"
                winnings = self.current_bet * 2
                self.budget += winnings
                self.wins += 1
                print(f"[RESULT] Dealer Busts! Player Wins ${winnings}")
            elif player_value > dealer_value:
                self.result_message = "You Win!"
                winnings = self.current_bet * 2
                self.budget += winnings
                self.wins += 1
                print(f"[RESULT] Player Wins ${winnings}")
            elif player_value < dealer_value:
                self.result_message = "Dealer Wins!"
                self.losses += 1
                print(f"[RESULT] Dealer Wins | Player loses ${self.current_bet}")
            else:
                self.result_message = "Push (Draw)"
                self.budget += self.current_bet
                self.draws += 1
                print(f"[RESULT] Push (Draw) | Bet returned: ${self.current_bet}")
        
        print(f"\n[STATS] Budget: ${self.budget} | W/L/D: {self.wins}/{self.losses}/{self.draws}")
        print("=" * 60)
